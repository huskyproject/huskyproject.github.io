#include "top.h"
#include "navbar.h"

      <h3>What is it?</h3>
      <blockquote>
        <strong>hpt</strong> stands for Highly Portable Tosser or High Performance Tosser, whichever you like :-). It is a full-featured FTN
	tosser.
      </blockquote>

      <h3>What does it need?</h3>
      <blockquote>
        Operating systems:
        <ul>
          <li>All supported platforms.
        </ul>
        Dependencies:
        <ul>
          <li>hpt-current requires fidoconf-current, smapi-current and huskybse-current from the same day</li>
	<li>hpt-0.9.7a-stable requires fidoconf-0.10-stable, smapi-1.6.4-stable(1.6.4a for JAM), and huskybse-0.1-stable</li>
          <li>hpt 0.31 requires fidoconfig 0.9 and smapi 1.6.2</li>
        </ul>
      </blockquote>

      <h3>Downloads</h3>
      <blockquote>
        <table cellpadding="5" cellspacing="0" border="0">
          <tr valign="top">
            <td><a href="http://www.physcip.uni-stuttgart.de:8080/fidosoft.husky/hpt-0.9.7a-src.tar.gz"><tt>hpt-0.9.7a-src.tar.gz</tt></a>
            <br><a href="http://www.physcip.uni-stuttgart.de:8080/fidosoft.husky/hpt-0.9.7a-src.zip"><tt>hpt-0.9.7a-src.zip</tt></a>
	    </td>
            <td>current release: hpt 0.9.7a-stable source code</td>
          </tr>
          <tr valign="top">
             <td><a href="http://www.physcip.uni-stuttgart.de:8080/fidosoft.husky/hpt-0.9.7a-os2.zip"><tt>hpt-0.9.7a-os2.zip</tt></a>
             </td>
             <td>hpt 0.9.7a-stable os/2 executables</td>
          </tr>
          <tr valign="top">
             <td><a href="http://www.physcip.uni-stuttgart.de:8080/fidosoft.husky/hpt-0.9.7a-win.zip"><tt>hpt-0.9.7a-win.zip</tt></a>
             </td>
             <td>hpt 0.9.7a-stable win32 executables</td>
          </tr>
          <tr valign="top">
            <td><a href="http://www.physcip.uni-stuttgart.de:8080/fidosoft.husky/hpt-0.31.tar.gz"><tt>hpt-0.31.tar.gz</tt></a></td>
            <td>old release: hpt 0.31 source code</td>
          </tr>
          <tr valign="top">
            <td><a href="http://cvs.physcip.uni-stuttgart.de/cvs2/hpt-latest.tar.gz"><tt>hpt-latest.tar.gz</tt></a></td>
            <td>unstable development cvs snapshot</td>
          </tr>
        </table>
      </blockquote>

      <h3>Contacting the author</h3>
      <blockquote>
        Maintained by Matthias Tichy, <a href="mailto:sackett@mail.ru">Max Levenkov</a> et al. See the <a href="team.html">team page</a>.
      </blockquote>


      <small>Last update: 21/07/2000</small>

#include "bottom.h"
